...
│   ├── manifest.json
│   ├── icons/
│   ├── robots.txt                # AI-friendly robots.txt for birthbuild.com
│   ├── llms.txt                  # llms.txt for birthbuild.com
│   └── sitemap.xml               # Root sitemap (generated by blog build, gitignored)
├── src/
│   ├── main.tsx
│   ├── App.tsx
│   ├── routes/
...
│   │   ├── schema-generators.ts   # Schema.org JSON-LD builders (LocalBusiness, Service, Review, Credential)
│   │   ├── seo-files.ts           # Shared robots.txt + sitemap.xml generators (AI crawler allows, lastmod)
│   │   └── llms-txt.ts            # llms.txt generator from SiteSpec (llmstxt.org Markdown spec)
│   ├── hooks/
...
- **HTML/CSS sanitisation** — All LLM-generated HTML is sanitised via regex-based stripping of `<script>`, event handlers, `javascript:` URLs, `<base>` tags, CSS `@import`/`expression()`/`url()` injection. `<script type="application/ld+json">` blocks are preserved via extract → JSON round-trip validation → restore.
...
- **Blog SEO** — Each article page includes Article JSON-LD, BreadcrumbList JSON-LD, and (if `faq` present) FAQPage JSON-LD. Index page has Open Graph + Twitter Card meta. Blog sitemap at `/blog/sitemap.xml`. Blog build also generates root `public/sitemap.xml` combining landing page + all blog posts + llms.txt.
...
- **Agentic SEO / GEO** — Generated sites include: entity-rich h1 tags (business name + keyword + location), answer-first hero paragraphs, full Schema.org JSON-LD per page (LocalBusiness+Person on home, Service on services, Review+AggregateRating on testimonials, Person+Credential on about, FAQPage on FAQ), AI-friendly robots.txt (explicit allows for GPTBot/ClaudeBot/PerplexityBot/Google-Extended/Applebot-Extended), llms.txt (Markdown summary from SiteSpec), sitemap.xml with `<lastmod>`. Schema builders shared via `schema-generators.ts`. Robots/sitemap shared via `seo-files.ts` (both build paths import, no duplication).
- **birthbuild.com SEO** — Landing page (`index.html`) has OG tags, Twitter Cards, canonical URL, SoftwareApplication JSON-LD. `public/robots.txt` and `public/llms.txt` served statically. Root `public/sitemap.xml` generated by `blog/build-blog.ts` combining landing + blog + llms.txt URLs.
